        ...
        <div class="bg-white border rounded shadow-sm p-4 max-w-xl mx-auto">
            ...
            @php
                ...
                $checkinsJson = collect($checkinsByDate)->map(function($col) {
                    return collect($col)->map(function($c){
                        $arr = $c->toArray();
                        $arr['deletable'] = \Carbon\Carbon::now()->diffInSeconds($c->created_at) <= (10 * 3600);
                        return $arr;
                    })->values()->all();
                })->all();
                        ...
                        <form method="POST" action="{{ route('checkin.store') }}" class="flex flex-col flex-1 min-h-0">
                            ...
                            <div class="px-4 py-3 bg-white border-t flex justify-between items-center gap-2 flex-shrink-0 sticky bottom-0 z-10">
                                <div class="flex items-center gap-2">
                                    <form x-show="existingId && canDelete" x-bind:action="'/checkin/' + existingId" method="POST" class="mr-2" onsubmit="return confirm('Delete this check-in? This cannot be undone.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-3 py-1 bg-red-600 text-white rounded">Delete</button>
                                    </form>
                                    <div x-show="existingId && !canDelete" class="text-xs text-gray-500">Deletion window expired (10 hours)</div>
                                </div>
